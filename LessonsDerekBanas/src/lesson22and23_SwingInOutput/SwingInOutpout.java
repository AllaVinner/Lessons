package lesson22and23_SwingInOutput;
import javax.swing.*;
...
public class SwingInOutpout extends JFrame{
	...
	public SwingInOutpout() {
		...
		howManyTimes = new JSlider(0, 10, 1);
		howManyTimes.setMinorTickSpacing(1);
		howManyTimes.setMajorTickSpacing(5);
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource() == button1){
				...
				try {
					...
					JOptionPane.showMessageDialog(SwingInOutpout.this, "Please Enter the Right Infoe", "Error", JOptionPane.ERROR_MESSAGE);
					System.exit(0);
				}
				
				if(addNums.isSelected())		{totalCalc = addNumbers(number1, number2, howManyTimes.getValue());}
				if(subtractNums.isSelected())	{totalCalc = subtractNumbers(number1, number2, howManyTimes.getValue());}
				if(multiNums.isSelected()) 		{totalCalc = multiplyNumbers(number1, number2, howManyTimes.getValue());}
				if(divideNums.isSelected())		{totalCalc = divideNumbers(number1, number2, howManyTimes.getValue());
				}
			}
			
			if(dollarSign.isSelected()){
				NumberFormat numFormat = NumberFormat.getCurrencyInstance();
				JOptionPane.showMessageDialog(SwingInOutpout.this, numFormat.format(totalCalc), "Solution", JOptionPane.INFORMATION_MESSAGE);
				...
				JOptionPane.showMessageDialog(SwingInOutpout.this, totalCalc, "Solution", JOptionPane.INFORMATION_MESSAGE);		
			}
